...
除了方法实现与资源管理, 主要成员为两个指针(因此共享指针的内存占用是普通指针的两倍), 一个为普通指针，另一个指针用于计数

一般64位系统中shared_ptr占用16个字节，一个指针8个字节

不要混用智能指针和普通指针去指向同一个资源，容易出现智能指针释放了资源，但普通指针不知道，出现未定义的空悬指针

##### unique_ptr

##### weak_ptr

weak_ptr的出现针对的是shared_ptr中循环引用的问题
